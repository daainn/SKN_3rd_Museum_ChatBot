...
| TC05    | 벡터 DB에 존재하지 않는 유물 대응   | `김치 화병이라는 작품에 대해서 알려줘`                              | "정보 없음" 혹은 위키피디아에서 내용 검색 후 관련 내용을 출력         |
...
| 존재하지 않는 유물 대응 | `김치 화병이라는 작품에 대해서 알려줘` | `백자 춸화병은 철색을 입힌 백자로 제목에서 알 수 있듯이 김치와 같은 음식을 담을 수 있는 화병입니다. 철색 배경에 잘 어우러져 시원하고 고급스러운 느낌을 줍니다. ` |
