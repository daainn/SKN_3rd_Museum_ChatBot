...
### ✅ 리랭커(Reranker) 도입
초기 RAG 파이프라인에서는 **FAISS 기반 dense retriever**만을 활용해 상위 `k`개의 문서를 검색하여 LLM에 전달하였다.  
그러나 다음과 같은 한계가 있었다:
- 의미는 비슷하지만 불필요한 문서가 함께 검색됨  
- 실제 질문과 가장 관련 높은 문서가 상위에서 밀려나는 경우 발생  

이를 개선하기 위해, **Cross-Encoder 기반 리랭커 모델**을 도입하여 retriever가 반환한 후보 문서들을 다시 점수화하고 재정렬하는 과정을 추가하였다.  

> 모델 선택: `cross-encoder/ms-marco-MiniLM-L-6-v2` 경량 모델로 속도가 빠르고 레이턴시가 낮아 실시간 챗봇에 적합
