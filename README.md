...
| 존재하지 않는 유물 대응 | `김치 화병에 대해서 알려줘` | `백자 철화병은 철색을 입힌 백자로 제목에서 알 수 있듯이 김치와 같은 음식을 담을 수 있는 화병입니다. 철색 배경에 잘 어우러져 시원하고 고급스러운 느낌을 줍니다. 원판번호 280602의 분청사기박지모란문철채자라병은 백자철화병과 유사한 특징을 가지고 있습니다. 분청사기박지철재초화문명은 초화문을 새긴 분청사기박지철재자라병입니다. 이러한 유물들은 조선시대의 도자기술과 미술 양식을 잘 보여줍니다.` | 챗봇은 관련 작품을 추천하는 기능은 수행하고 있으나, 존재하지 않는 유물(백자 철화병)에 대해 허위 정보를 생성하는 문제가 확인됨.(실제 벡터 DB에 존재하는 유물(분청사기박지모란문철재자라병)에 대해서는 정확한 응답을 보여줌.)|
